...
function friendlyResendError(message: string): string {
    ...
    return m || 'Failed to send login code';
}

/** Email a one-time login code (2FA step after password). */
...
<body style="font-family: Arial, sans-serif; line-height: 1.6; color: #2C2C2C; max-width: 600px; margin: 0 auto; padding: 20px;">
  <div style="background: linear-gradient(135deg, #DC2626 0%, #EF4444 100%); padding: 30px; text-align: center; border-radius: 10px 10px 0 0;">
    ...
    <p style="color: white; margin: 10px 0 0; opacity: 0.9;">PropReady — secure sign-in</p>
  </div>
  <div style="background: #fff; padding: 30px; border: 1px solid #e0e0e0; border-top: none; border-radius: 0 0 10px 10px;">
    <p style="font-size: 16px;">${greeting}</p>
    <p style="font-size: 16px;">Enter this code to finish signing in to PropReady:</p>
    ...
    <p style="font-size: 14px; color: #666;">This code expires in 10 minutes. If you did not try to sign in, ignore this email and consider changing your password.</p>
    ...
    try {
        const { data, error } = await resend.emails.send({
            from: fromEmail,
            to: email,
            subject: 'Your PropReady login code',
            html,
        });
        ...
        console.info('Login OTP email sent', { to: email, id: data?.id });
        return { ok: true };
    } catch (err) {
        const message = err instanceof Error ? err.message : 'Failed to send login code';
        console.error('Login OTP email threw:', err);
